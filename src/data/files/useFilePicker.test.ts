...
describe('useFilePicker', () => {
  ...
  it('should pick files successfully', async () => {
    ...
    const changeCallback = mockInput.addEventListener.mock.calls.find((call: any) => call[0] === 'change')?.[1]
  ...
  it('should handle file picker cancellation', async () => {
    ...
    const cancelCallback = mockInput.addEventListener.mock.calls.find((call: any) => call[0] === 'cancel')?.[1]
    cancelCallback()
  ...
  it('should handle file picker error', async () => {
    ...
    // Simulate error
    const errorCallback = mockInput.addEventListener.mock.calls.find((call: any) => call[0] === 'error')?.[1]
    errorCallback()
  ...
  it('should pick directory', async () => {
    ...
    expect(result).toEqual(mockFiles)
    expect(mockInput.setAttribute).toHaveBeenCalledWith('webkitdirectory', 'true')
  })
  ...
  it('should pick images', async () => {
    ...
    const changeEvent = { target: { files: mockFiles.filter(f => f.type.startsWith('image/')) } }
    const changeCallback = mockInput.addEventListener.mock.calls.find((call: any) => call[0] === 'change')?.[1]
    changeCallback(changeEvent)
