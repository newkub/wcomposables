...
export interface FilterOption<T> {
  key: keyof T
  value: any
  operator?: 'equals' | 'contains' | 'startsWith' | 'endsWith' | 'gt' | 'gte' | 'lt' | 'lte' | 'in' | 'nin'
...
export function useFilter<T>(items: T[]) {
  ...
  const updateFilter = (key: keyof T, updates: Partial<FilterOption<T>>) => {
    ...
    if (filterIndex >= 0) {
      filters.value[filterIndex] = { ...filters.value[filterIndex], ...updates } as any
    }
  }
  ...
  const filterBy = (key: keyof T, value: any, operator?: FilterOption<T>['operator'], compareFn?: (item: T[keyof T], value: any) => boolean) => {
