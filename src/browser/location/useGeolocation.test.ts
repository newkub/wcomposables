...
describe('useGeolocation', () => {
  ...
  it('should handle different error codes', async () => {
    ...
    // Test error code 1
    navigator.geolocation.getCurrentPosition = vi.fn((error) => error({ code: 1 }))
    getCurrentPosition()
    await vi.runAllTimersAsync()
    expect(error.value).toBe('Location permission denied')

    // Test error code 2
    navigator.geolocation.getCurrentPosition = vi.fn((error) => error({ code: 2 }))
    getCurrentPosition()
    await vi.runAllTimersAsync()
    expect(error.value).toBe('Location position unavailable')

    // Test error code 3
    navigator.geolocation.getCurrentPosition = vi.fn((error) => error({ code: 3 }))
    getCurrentPosition()
    await vi.runAllTimersAsync()
