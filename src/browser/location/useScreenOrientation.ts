...
export type OrientationType = 'portrait-primary' | 'portrait-secondary' | 'landscape-primary' | 'landscape-secondary' | 'any'
...
export function useScreenOrientation(options: UseScreenOrientationOptions = {}) {
  ...
  const lock = async (lockType: OrientationLockType): Promise<boolean> => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to lock orientation'
      error.value = errorMsg
      onError?.(errorMsg)
      return false
    }
  ...
  const unlock = async (): Promise<boolean> => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to unlock orientation'
      error.value = errorMsg
      onError?.(errorMsg)
      return false
    }
