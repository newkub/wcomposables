...
import { useScreenOrientation, useDeviceOrientation } from './useScreenOrientation'
...
describe('useScreenOrientation', () => {
  ...
  it('should initialize with default values', () => {
    const { isSupported, orientation, angle, isLocked, error } = useScreenOrientation()
  ...
  it('should provide orientation computed properties', () => {
    ...
    const { isPortrait, isLandscape, isPrimary, isSecondary } = useScreenOrientation()
  ...
  it('should handle orientation change events', () => {
    ...
    const changeHandler = (screen.orientation.addEventListener as any).mock.calls?.find((call: any) => call[0] === 'change')?.[1]
  ...
  it('should get orientation and angle', () => {
    const { getOrientation, getAngle, updateOrientation } = useScreenOrientation()
...
describe('useDeviceOrientation', () => {
  ...
  it('should initialize with default values', () => {
    const { isSupported, absolute, alpha, beta, gamma, heading, error } = useDeviceOrientation()
  ...
  it('should request permission successfully', async () => {
    ...
    vi.mocked(DeviceOrientationEvent as any).requestPermission = vi.fn().mockResolvedValue('granted')
  ...
  it('should handle permission denial', async () => {
    vi.mocked(DeviceOrientationEvent as any).requestPermission = vi.fn().mockResolvedValue('denied')
    ...
    expect(onError).toHaveBeenCalledWith('Device orientation permission denied')
  })

  it('should handle permission request error', async () => {
    vi.mocked(DeviceOrientationEvent as any).requestPermission = vi.fn().mockRejectedValue(new Error('Permission error'))
  ...
  it('should handle device orientation events', () => {
    ...
    const { alpha, beta, gamma, heading } = useDeviceOrientation({ onAlpha, onBeta, onGamma, onHeading })

    // Find the orientation event handler
    const orientationHandler = (window.addEventListener as any).mock.calls?.find((call: any) => call[0] === 'deviceorientation')?.[1]
  ...
  it('should clear values', () => {
    const { absolute, alpha, beta, gamma, heading, error, clear } = useDeviceOrientation()
