...
export function useClipboard(options: ClipboardOptions = {}) {
  ...
  const copyElement = async (element: Element): Promise<boolean> => {
    try {
      const textToCopy = element.textContent || (element as HTMLElement).innerText || ''
      return copy(textToCopy)
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Failed to copy element content'
      return false
    }
  }

  const copyElementValue = async (element: HTMLInputElement | HTMLTextAreaElement): Promise<boolean> => {
    try {
      return copy(element.value)
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Failed to copy element value'
      return false
    }
  }
  ...
  const copyRichText = async (textContent: string, html: string): Promise<boolean> => {
    if (!isSupported.value) {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to copy rich text'
      return false
    }
  }
