...
export function useLanguage(options: LanguageOptions = {}) {
  ...
  const getLanguageDirection = (locale: string = language.value): 'ltr' | 'rtl' => {
    return isRTL(locale) ? 'rtl' : 'ltr'
  }

  const formatDate = (date: Date, locale: string = language.value, options?: Intl.DateTimeFormatOptions) => {
    return new Intl.DateTimeFormat(locale, options).format(date)
  }

  const formatNumber = (num: number, locale: string = language.value, options?: Intl.NumberFormatOptions) => {
    return new Intl.NumberFormat(locale, options).format(num)
  }

  const formatCurrency = (amount: number, currency: string, locale: string = language.value) => {
    return new Intl.NumberFormat(locale, { style: 'currency', currency }).format(amount)
  }

  const formatRelativeTime = (value: number, unit: Intl.RelativeTimeFormatUnit, locale: string = language.value) => {
