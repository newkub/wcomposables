...
export function useWebSocket(options: WebSocketOptions) {
  ...
  const connect = () => {
    ...
    try {
      websocket = protocols ? new WebSocket(url, protocols) : new WebSocket(url)
  ...
  const getMessagesByType = (type: string): WebSocketMessage[] => {
    return messageHistory.value.filter(msg => 
      typeof msg.data === 'object' && msg.data.type === type
    )
  }

  const getMessagesInTimeRange = (startTime: number, endTime: number): WebSocketMessage[] => {
    return messageHistory.value.filter(msg => 
