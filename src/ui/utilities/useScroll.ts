...
export function useScroll(options: UseScrollOptions = {}) {
  ...
  const scrollTo = (options?: ScrollToOptions | { x?: number; y?: number; behavior?: ScrollBehavior }) => {
    if (isElement(target)) {
  ...
  const scrollBy = (options?: ScrollToOptions | { x?: number; y?: number; behavior?: ScrollBehavior }) => {
    if (isElement(target)) {
      if (options && 'x' in options) {
        target.scrollBy({
          left: options.x,
