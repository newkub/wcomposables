...
export function useNotification(options: UseNotificationOptions = {}) {
  ...
  const show = (notification: Omit<Notification, 'id' | 'timestamp'>): string => {
    const id = generateId()
  ...
  const success = (title: string, message: string, options?: Partial<Omit<Notification, 'id' | 'timestamp' | 'title' | 'type' | 'message'>>) => {
    return show({ title, message, type: 'success', ...options })
  }

  const error = (title: string, message: string, options?: Partial<Omit<Notification, 'id' | 'timestamp' | 'title' | 'type' | 'message'>>) => {
    return show({ title, message, type: 'error', persistent: true, ...options })
  }

  const warning = (title: string, message: string, options?: Partial<Omit<Notification, 'id' | 'timestamp' | 'title' | 'type' | 'message'>>) => {
    return show({ title, message, type: 'warning', ...options })
  }

  const info = (title: string, message: string, options?: Partial<Omit<Notification, 'id' | 'timestamp' | 'title' | 'type' | 'message'>>) => {
    return show({ title, message, type: 'info', ...options })
  }

  const update = (id: string, updates: Partial<Omit<Notification, 'id' | 'timestamp'>>) => {
